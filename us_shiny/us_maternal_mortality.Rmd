...
cdc_maternal_death_data = read.delim("~/Documents/MS_Biostatistics/P8105/Final_Project/p8105_final.github.io/us_shiny/data/Underlying_Cause_of_Death_1999-2017.txt") %>%
  janitor::clean_names() %>%
  na.omit() %>%
...
cdc_birth_data = read.csv("~/Documents/MS_Biostatistics/P8105/Final_Project/p8105_final.github.io/us_shiny/data/us_states_data.csv") %>%
  janitor::clean_names() %>%
  na.omit() %>%
...
cdc_merged_data = cdc_maternal_death_data %>%
  group_by(state) %>%
  summarise(total_death = sum(deaths)) %>%
  na.omit() %>%
